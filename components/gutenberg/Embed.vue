...
export default {
  ...
  computed: {
    ...
    isVimeo() {
      return this.provider == "vimeo" || String(this.url).includes("vimeo.com");
    },
    isShowcase() {
      return (
        this.provider == "vimeo" &&
        String(this.url).includes("vimeo.com/showcase")
      );
    },
    isYouTube() {
      return (
    ...
    iFrameSrc() {
      ...
      switch (true) {
        case this.isShowcase:
          url += `/embed?&byline=0&portrait=0&autoplay=false&playsinline=true&color=${this.color}`;
          break;
